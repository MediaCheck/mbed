...
#ifndef EMAC_AUTONEGO_ENABLED
#define EMAC_AUTONEGO_ENABLED   0
#endif

#ifndef EMAC_DEBUG_ENABLED
#define EMAC_DEBUG_ENABLED      0
#endif

#ifndef BYZANCE_USE_ETH_HEAP
#define BYZANCE_USE_ETH_HEAP
#endif

#if EMAC_DEBUG_ENABLED
    #define EMAC_DEBUG(...);    printf(__VA_ARGS__);
#else
    #define EMAC_DEBUG(...);    {}
#endif

//BYZANCE HACK
#if defined(BYZANCE_USE_ETH_HEAP)
//allocate pointers in .bss 
...
bool STM32_EMAC::low_level_init_successful()
{
    ...
    #if defined(BYZANCE_USE_ETH_HEAP)
    uint8_t* tempTX;
    Tx_Buff = (uint8_t**)malloc(ETH_TXBUFNB * sizeof(uint8_t*));
    tempTX = (uint8_t*)malloc(ETH_TXBUFNB * ETH_TX_BUF_SIZE);
    if(tempTX ==0 || Tx_Buff==0){
        EMAC_DEBUG("error allocate TX buf - is .bss in CCM?\n");
        return false;
    }
    for (int i = 0; i < ETH_TXBUFNB; i++) {
      Tx_Buff[i] = tempTX + (i * ETH_TX_BUF_SIZE);
    }
    DMATxDscrTab=(ETH_DMADescTypeDef *)malloc(ETH_TXBUFNB*sizeof(ETH_DMADescTypeDef));
    if(DMATxDscrTab==0){
        EMAC_DEBUG("TX DMA malloc failed - is .bss in CCM?\n");
        return false;
    }
    #endif
    //END OF BYZANCE HACK
    ...
    #if defined(BYZANCE_USE_ETH_HEAP)
    uint8_t* tempRX;
    Rx_Buff = (uint8_t**)malloc(ETH_RXBUFNB * sizeof(uint8_t*));   //malloc pointers to buffers
    tempRX = (uint8_t*)malloc(ETH_RXBUFNB * ETH_RX_BUF_SIZE);     //malloc buffers
    if(tempRX == 0 || Rx_Buff==0){     //check buffers
        EMAC_DEBUG("error allocate RX buf - is .bss in CCM?\n");
        return false;
    }
    //fill pointers
    for (int i = 0; i < ETH_RXBUFNB; i++) {
      Rx_Buff[i] = tempRX + (i * ETH_RX_BUF_SIZE);
    }
    //malloc DMA description table
    DMARxDscrTab=(ETH_DMADescTypeDef *)malloc(ETH_RXBUFNB*sizeof(ETH_DMADescTypeDef));
    if(DMARxDscrTab==0){     //check it
        EMAC_DEBUG("RX DMA malloc failed - is .bss in CCM?\n");
        return false;
    }
    #endif
    //END OF BYZANCE HACK
