                                                    ...
                                                    nsapi_ip_stack_t stack, bool blocking)
{
    ...
    if(!connect_called){
        ...
        if((_connect_status==NSAPI_STATUS_LOCAL_UP)||(_connect_status==NSAPI_STATUS_GLOBAL_UP)){
            return NSAPI_ERROR_OK;
        } else {
...
//BYZANCE HACK
void LoWPANNDInterface::set_credentials(char *name, uint8_t key[16]){
    nd_tasklet_set_credentials(name,key);
}
//END OF BYZANCE HACK
