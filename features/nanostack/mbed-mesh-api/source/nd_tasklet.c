...
//BYZANCE HACK
static uint8_t nwk_key[16]={0xa0,0xa1,0xa2,0xa3,0xa4,0xa5,0xa6,0xa7,0xa8,0xa9,0xaa,0xab,0xac,0xad,0xae,0xaf};
static uint8_t nwk_name[16];
//END OF BYZANCE HACK
...
void nd_tasklet_configure_and_connect_to_network(void)
{
    ...
    if (strcmp(sec_mode, "PSK") == 0) {
        ...
        //BYZANCE HACK
        memcpy(tasklet_data_ptr->psk_sec_info.security_key, nwk_key, 16);
        //END OF BYZANCE HACK
    } else {
    ...
    arm_nwk_6lowpan_link_nwk_id_filter_for_nwk_scan(
        tasklet_data_ptr->network_interface_id, nwk_name);
    //END OF BYZANCE HACK
...
//BYZANCE HACK
void nd_tasklet_set_credentials(char *name, uint8_t *k){
    memcpy(nwk_key,k,16);
    strncpy(nwk_name,name,16);
}
//END OF BYZANCE HACK
